  ...
  return (
    <div className="flex flex-col h-screen bg-white overflow-hidden">
      ...
      <div className="flex flex-1 min-h-0">
        {/* LEFT PANEL */}
        <div className="flex flex-col w-[680px] flex-shrink-0 border-r border-gray-100">
          {/* Tab nav */}
          ...
          <div className="flex-1 min-h-0 overflow-y-auto px-8 py-6">
            ...
            {activeTab === 'settings' && (
              <div>
                <h2 className="text-gray-900 font-bold text-lg mb-1">캐릭터 설정</h2>
            ...
            {activeTab === 'intro' && (
              <div>
                <h2 className="text-gray-900 font-bold text-lg mb-1">인트로 및 예시 대화</h2>
                ...
                <div className="border border-gray-200 rounded-2xl overflow-hidden">
                  ...
                  <div className="p-6">
                    ...
                    <button
                      onClick={() => introInputRef.current?.focus()}
            ...
            {activeTab === 'prompt' && (
              <div>
                <h2 className="text-gray-900 font-bold text-lg mb-1">프롬프트</h2>
            ...
            {activeTab === 'advanced' && (
              <div>
                <h2 className="text-gray-900 font-bold text-lg mb-1">고급 기능</h2>
            ...
            {activeTab === 'detail' && (
              <div>
                <h2 className="text-gray-900 font-bold text-lg mb-1">캐릭터 상세</h2>
                <p className="text-gray-400 text-sm mb-6">공개 범위와 연령 등급을 설정해요.</p>
                <div className="border border-gray-200 rounded-2xl divide-y divide-gray-100">
                  {/* 공개 범위 */}
                  <div className="p-6">
                    <span className="text-gray-900 font-semibold text-sm block mb-3">공개 범위</span>
                    <div className="space-y-2">
                      {[
                        { value: 'PUBLIC', label: '공개', desc: '누구나 이 캐릭터와 대화할 수 있어요' },
                        { value: 'PRIVATE', label: '비공개', desc: '나만 볼 수 있어요' },
                        { value: 'UNLISTED', label: '링크 공개', desc: '링크를 가진 사용자만 접근할 수 있어요' },
                      ].map((opt) => (
                        <label key={opt.value} className={cn('flex items-center gap-3 p-4 rounded-xl border cursor-pointer transition-all',
                          formData.visibility === opt.value ? 'bg-gray-50 border-gray-400' : 'border-gray-200 hover:bg-gray-50')}>
                          <input type="radio" name="visibility" value={opt.value} checked={formData.visibility === opt.value}
                            onChange={() => update({ visibility: opt.value as any })} className="accent-gray-900" />
                          <div>
                            <p className={cn('text-sm font-medium', formData.visibility === opt.value ? 'text-gray-900' : 'text-gray-700')}>{opt.label}</p>
                  ...
                  {/* 연령 등급 */}
                  <div className="p-6">
                    <span className="text-gray-900 font-semibold text-sm block mb-3">연령 등급</span>
                    <div className="space-y-2">
                      {[
                        { value: 'ALL', label: '전체 이용가', desc: '모든 연령대가 이용할 수 있어요' },
                        { value: 'MATURE', label: '성인', desc: '성인 인증이 필요해요' },
                      ].map((opt) => (
                        <label key={opt.value} className={cn('flex items-center gap-3 p-4 rounded-xl border cursor-pointer transition-all',
                          formData.ageRating === opt.value ? 'bg-gray-50 border-gray-400' : 'border-gray-200 hover:bg-gray-50')}>
                          <input type="radio" name="ageRating" value={opt.value} checked={formData.ageRating === opt.value}
                            onChange={() => update({ ageRating: opt.value as any })} className="accent-gray-900" />
                          <div>
                            <p className={cn('text-sm font-medium', formData.ageRating === opt.value ? 'text-gray-900' : 'text-gray-700')}>{opt.label}</p>
                            <p className="text-xs text-gray-400">{opt.desc}</p>
                          </div>
                        </label>
                      ))}
                  ...
                  {/* 태그 */}
                  <div className="p-6">
                    <span className="text-gray-900 font-semibold text-sm block mb-1">태그 <span className="text-gray-400 font-normal">({formData.tags.length}/10)</span></span>
                    <p className="text-gray-400 text-xs mb-3">Enter로 태그를 추가하세요</p>
                    ...
                    <input type="text" value={formData.tagInput}
                      onChange={(e) => update({ tagInput: e.target.value })}
                      onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ',') { e.preventDefault(); addTag(formData.tagInput); } }}
                      placeholder="태그 입력 후 Enter"
                      className="w-full px-4 py-3 rounded-xl border border-gray-200 text-gray-900 text-sm placeholder:text-gray-300 focus:outline-none focus:border-gray-400" />
                  </div>
                </div>
              </div>
            )}
        ...
        {activeTab === 'intro' ? (
          ...
          /* ── 캐릭터 설정 / 캐릭터 상세 탭: 인기 캐릭터 모아보기 ── */
          <div className="flex-1 min-w-0 px-8 py-6 overflow-y-auto">
            <h3 className="text-gray-900 font-bold text-base mb-4">인기 캐릭터 모아보기</h3>
